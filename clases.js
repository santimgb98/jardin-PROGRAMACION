...
// Función asíncrona para regar la planta que lo necesite
    async function regarPlanta(nombre_planta) {
        ...
        return new Promise((resolve) => {
            ...
            resolve(fs.writeFileSync("./data.json",(JSON.stringify(plantasBDjson, null, 2)), 'utf8'));
...
class Jardin{
    ...
    async addPlanta(){
        ...
        while(isNaN(this.frecuencia_riego)===true || this.frecuencia_riego < 1){
            this.frecuencia_riego = Number(prompt(`Cada cuantos días (número) se riega ${this.nombre}: `));
        }
    ...
    async necesarioRiegoSiNo(){
        ...
        if(contador >0){
            let opcion = prompt('¿Quieres regar las plantas que lo necesitan? (si/no): ');
        ...
        } else{console.log('No hay plantas que regar')}
